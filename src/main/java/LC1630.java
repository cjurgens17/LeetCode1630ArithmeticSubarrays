...
public class LC1630 {
    ...
    // l and r, representing the m range queries, where the ith query is the range [l[i], r[i]]. All the arrays are 0-indexed.
    //
    ...
    public static List<Boolean>  checkArithmeticSubarrays(int[] nums, int[] l, int[] r){
        if(nums.length < 2 || nums == null) return new ArrayList<>();

        boolean[] valid = new boolean[l.length];
        ...
        while(count < l.length){
            ...
            int[] temp = new int[r[count] - l[count]+1];
            ...
            Arrays.sort(temp);
            int slice = temp[1] - temp[0];
            int check = slice;

            for(int i = 1;i<temp.length;i++){
                slice = temp[i] - temp[i-1];
                if(check != slice){
                    valid[count] = false;
                }
            }
